...
class _AddressBookState extends State<AddressBook> {
  ...
  void _showMenu(RelativeRect pos) {
    ...
    final items = [
      ...
      getEntry(translate("Export address book"), () async {
        final jsonData = await gFFI.abModel.exportAddressBook();
        if (jsonData.isNotEmpty) {
          final filePath = await bind.mainSaveFileDialog(
            title: "Export Address Book",
            defaultName: "address_book.json",
            filters: ["json"],
          );
          if (filePath != null && filePath.isNotEmpty) {
            try {
              await File(filePath).writeAsString(jsonData);
              BotToast.showText(contentColor: Colors.green, text: translate("Export successful"));
            } catch (e) {
              BotToast.showText(contentColor: Colors.red, text: "Export failed: $e");
            }
          }
        }
      }),
      getEntry(translate("Import address book"), () async {
        final filePath = await bind.mainOpenFileDialog(
        ...
        if (filePath != null && filePath.isNotEmpty) {
          try {
            ...
            final success = await gFFI.abModel.importAddressBook(jsonData);
            if (success) {
              BotToast.showText(contentColor: Colors.green, text: translate("Import successful"));
            } else {
              BotToast.showText(contentColor: Colors.red, text: translate("Import failed"));
            }
          } catch (e) {
            BotToast.showText(contentColor: Colors.red, text: "Import failed: $e");
          }
        }
      }),
    ];
