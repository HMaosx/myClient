...
class GroupModel {
  ...
  Future<String> exportGroups() async {
    try {
      final groupData = {
        'version': '1.0',
        'timestamp': DateTime.now().toIso8601String(),
        'device_groups': deviceGroups.map((group) => group.toGroupCacheJson()).toList(),
        'users': users.map((user) => user.toGroupCacheJson()).toList(),
        'peers': peers.map((peer) => peer.toGroupCacheJson()).toList()
      };
      return jsonEncode(groupData);
    } catch (e) {
      debugPrint('export groups error: $e');
      return '';
    }
  }

  Future<bool> importGroups(String jsonData) async {
    try {
      ...
      if (data['device_groups'] is List) {
        deviceGroups.clear();
        for (var groupData in data['device_groups']) {
          if (groupData is Map) {
            deviceGroups.add(DeviceGroupPayload.fromJson(groupData));
          }
        }
      }
      ...
      if (data['users'] is List) {
        ...
        for (var userData in data['users']) {
          if (userData is Map) {
            users.add(UserPayload.fromJson(userData));
          }
        }
      }
      ...
      if (data['peers'] is List) {
        peers.clear();
        for (var peerData in data['peers']) {
          if (peerData is Map) {
            peers.add(Peer.fromJson(peerData));
          }
        }
        _callbackPeerUpdate();
      }
      ...
      debugPrint('import groups error: $e');
      return false;
    }
  }
}
