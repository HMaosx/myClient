...
// #region ab
  ...
  Future<void> _pullAb(
    ...
    if (!listInitialized || force == ForcePullAb.listAndCurrent) {
      try {
        // Local only mode - always use legacy address book
        ...
        if (!addressbooks.containsKey(_currentName.value)) {
          setCurrentName(_legacyAddressBookName);
        }
        // load from cache
        await loadCache();
        // mark as initialized
...
// #region tools
  ...
  Future<String> exportAddressBook() async {
    try {
      final addressBookData = {
        'version': '1.0',
        'timestamp': DateTime.now().toIso8601String(),
        'addressbooks': addressbooks.map((key, value) {
          return MapEntry(key, {
            'tags': value.tags,
            'peers': value.peers.map((peer) => peer.toJson()).toList(),
            'tag_colors': value.tagColors
          });
        })
      };
      return jsonEncode(addressBookData);
    } catch (e) {
      debugPrint('export address book error: $e');
      return '';
    }
  }

  Future<bool> importAddressBook(String jsonData) async {
    try {
      final data = jsonDecode(jsonData);
      if (data == null || data['addressbooks'] == null) {
        return false;
      }
      
      final addressbooksData = data['addressbooks'];
      if (addressbooksData is Map) {
        addressbooksData.forEach((key, value) {
          if (value is Map) {
            final abName = key.toString();
            final ab = addressbooks[abName] ?? LegacyAb();
            
            if (value['tags'] is List) {
              ab.tags.value = (value['tags'] as List).map((e) => e.toString()).toList();
            }
            
            if (value['peers'] is List) {
              ab.peers.clear();
              for (var peerData in value['peers']) {
                if (peerData is Map) {
                  ab.peers.add(Peer.fromJson(peerData));
                }
              }
            }
            
            if (value['tag_colors'] is Map) {
              ab.tagColors.value = Map<String, int>.from(value['tag_colors']);
            }
            
            addressbooks[abName] = ab;
          }
        });
      }
      ...
      debugPrint('import address book error: $e');
      return false;
    }
  }
...
class LegacyAb extends BaseAb {
  ...
  Future<bool> pullAbImpl({quiet = false}) async {
    ...
    try {
      ...
      if (cache.isNotEmpty) {
        final data = jsonDecode(cache);
        if (data != null && data['ab_entries'] is List) {
          final abEntries = data['ab_entries'];
          for (var entry in abEntries) {
            if (entry['name'] == _legacyAddressBookName) {
              if (entry['tags'] is List) {
                tags.value = (entry['tags'] as List).map((e) => e.toString()).toList();
              }
              if (entry['peers'] is List) {
                peers.clear();
                for (var peer in entry['peers']) {
                  peers.add(Peer.fromJson(peer));
                }
              }
              if (entry['tag_colors'] is String) {
                Map<String, dynamic> map = jsonDecode(entry['tag_colors']);
