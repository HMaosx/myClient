    ...
    return ChangeNotifierProvider.value(
        ...
        child: Consumer<ServerModel>(builder: ((context, model, child) {
          ...
          return _Card(title: 'Password', children: [
            ...
            if (usePassword)
              _SubButton('Set custom one-time password', () async {
                String currentCustomPassword = await bind.mainGetOption(key: 'custom-temporary-password');
                setCustomTemporaryPasswordDialog(context, currentCustomPassword, () async {
                  await model.updatePasswordModel();
                });
              }, tmpEnabled && !locked),
...
void changeSocks5Proxy() async {
  ...
  void setCustomTemporaryPasswordDialog(BuildContext context, String currentPassword, Function callback) {
    final p0 = TextEditingController(text: currentPassword);
    final p1 = TextEditingController(text: currentPassword);
    String errMsg0 = '';
    final maxLength = 128;

    gFFI.dialogManager.showCustomDialog(
      title: translate("Set Custom One-time Password"),
      content: ConstrainedBox(
        constraints: const BoxConstraints(minWidth: 500),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            Row(
              children: [
                Expanded(child: PasswordStrengthIndicator(password: p0.text)),
              ],
            ...
            Text(
              translate('Leave blank to use auto-generated password'),
              style: TextStyle(
                fontSize: 12,
                color: Colors.grey,
              ),
      ...
      actions: [
        dialogButton(
          translate("Cancel"),
          onPressed: () {
            gFFI.dialogManager.dismissCustomDialog();
          },
          isOutline: true,
        ),
        dialogButton(translate("OK"), onPressed: () async {
          final password = p0.text.trim();
          final confirm = p1.text.trim();
          if (password != confirm) {
            setState(() {
              errMsg0 = translate("The confirmation is not identical.");
            });
            return;
          }
          ...
          await bind.mainSetOption(key: 'custom-temporary-password', value: password);
          gFFI.dialogManager.dismissCustomDialog();
          callback();
        }),
      ],
